        ...
        const AudioEngine = {
            ...
            // New variables for the double-tap audio cheat
            lastToggleTime: 0,
            toggleCount: 0,
            ...
            toggle: function () {
                ...
                // --- Double-Tap Cheat Detection Logic ---
                const currentTime = performance.now();
                const timeDiff = currentTime - this.lastToggleTime;
                const COOLDOWN_MS = 2000;

                if (timeDiff < COOLDOWN_MS) {
                    this.toggleCount++;
                    if (this.toggleCount === 2) {
                        // CHEAT ACTIVATED: 2 toggles within 2 seconds
                        if (gameRunning && !ship.dead) {
                            if (typeof activateCheat === 'function') activateCheat();
                        }
                        this.toggleCount = 0; // Reset counter after cheat execution
                    }
                } else {
                    this.toggleCount = 1; // Start of a potential sequence
                }

                this.lastToggleTime = currentTime;
                // --- End Cheat Detection Logic ---

                // Standard audio toggle logic (mute/unmute)
                this.enabled = !this.enabled;
                if (this.enabled) {
                    if (this.ctx.state === 'suspended') this.ctx.resume();
                    this.masterGain.gain.setTargetAtTime(0.3, this.ctx.currentTime, 0.1);
                    // If the game is NOT running (menu/game over), resume music
                    if (!gameRunning) {
                        this.startMusic();
                    }
                } else {
        ...
        // NEW GLOBAL FLAG: To prevent click-to-mute after a successful hold cheat activation
        let cheatActivatedThisCycle = false;

        // Reusable cheat function (for double-tap)
        function activateCheat() {
            // Increment score to trigger next tier/evolution
            score = (Math.floor(score / EVOLUTION_SCORE_STEP) + 1) * EVOLUTION_SCORE_STEP;
            scoreEl.innerText = score;
            // Visual and sound feedback
            const vpX = width / 2; const vpY = height / 2;
            createExplosion(vpX, vpY, 60, '#00ff00', 4, 'spark');
            AudioEngine.playExplosion('large'); // SFX CHEAT
        }

        // Function to activate the "Epic Mode" cheat
        function activateEpicCheat() {
            if (!gameRunning || ship.dead) return;

            // 1. Set cheat flag to prevent accidental muting on button release
            cheatActivatedThisCycle = true;

            // 2. Set score to Tier 8 (Sphere)
            score = EVOLUTION_SCORE_STEP * 8;
            scoreEl.innerText = score;

            ship.maxShield = BASE_MAX_SHIELD + 50 * 5;
            ship.shield = ship.maxShield;

            // 3. Set Indestructible Flag (TRUE IMMUNITY, como solicitó el usuario)
            ship.invincible = true;

            // 4. Visual/Audio Feedback
            activateCheat(); // Uses existing explosion/audio for feedback

            // 5. ACTUALIZAR INDICADOR DE VIDAS A INFINITO
            drawLives();

            // 6. Message Animation
            epicMessageEl.innerText = "One small step for Aron, one giant leap for Mankind.";
            epicMessageEl.style.transition = 'none';
            epicMessageEl.style.opacity = '1';
            epicMessageEl.style.top = '70%'; // Start position (bottom)

            // Force a reflow to apply starting styles immediately
            void epicMessageEl.offsetWidth;

            // Animate (float up and fade out over 4s, transition opacity starts at 3s)
            epicMessageEl.style.transition = 'top 4s ease-out, opacity 1s linear 3s';
            epicMessageEl.style.top = '10%'; // End position (top)
            epicMessageEl.style.opacity = '0';

            // Clear message after animation
            setTimeout(() => {
                epicMessageEl.innerText = '';
                epicMessageEl.style.transition = 'none'; // Reset for next use
        ...
        const epicMessageEl = document.getElementById('epic-message');
        ...
        const BULLET_GRAVITY_FACTOR = 60;
        const BULLET_LIFETIME = 90;
        const BULLET_FADE_FRAMES = 3;

        // Lifetime and size for smaller side bullets
        const SIDE_BULLET_LIFETIME = 10;
        const PRIMARY_BULLET_SIZE = 3;
        const SECONDARY_BULLET_SIZE = 1.5;
        ...
        // Hold detection variables for Epic Cheat (Audio button)
        let audioHoldTimer = null;
        let isHoldingAudio = false;

        // Hold detection variables for Menu Cheat (START button)
        ...
        // --- Audio Button Listeners (Includes Cheat Logic) ---

        // 1. Double-tap cheat (handled by click event calling AudioEngine.toggle)
        audioBtn.addEventListener('click', (e) => {
            // CORRECCIÓN: Si el modo épico fue activado, prevenimos el evento de clic 
            // para evitar que el botón mutee el audio al soltar después de la pulsación larga.
            if (cheatActivatedThisCycle) {
                cheatActivatedThisCycle = false; // Reset the flag
                e.preventDefault();
        ...
        // 2. Hold cheat (Epic Mode) detection
        audioBtn.addEventListener('pointerdown', (e) => {
            if (e.button !== 0 && e.pointerType === 'mouse') return; // Only primary button
            if (!gameRunning || ship.dead) return;

            isHoldingAudio = true;
            // Start a timer for 2 seconds (2000 ms)
            audioHoldTimer = setTimeout(() => {
                if (isHoldingAudio) {
                    activateEpicCheat();
                    // We don't need to set cheatActivatedThisCycle here; it is set inside activateEpicCheat
                    isHoldingAudio = false; // Prevent repeated activation until the button is fully released and pressed again
                }
            }, 2000);
        });

        const stopHold = () => {
            // Clear timer and reset holding state when button is released or pointer leaves
            if (audioHoldTimer) {
                clearTimeout(audioHoldTimer);
                audioHoldTimer = null;
            }
            // Llamar a drawLives aquí para restablecer el indicador de vida si el truco épico se desactiva
            if (cheatActivatedThisCycle) {
                cheatActivatedThisCycle = false; // Reset the flag
                // No llamar a drawLives aquí, se manejará al inicio del juego/respawn
                // para evitar parpadeos si se mantiene el botón sin activar el truco.
            }
            isHoldingAudio = false;
        };

        audioBtn.addEventListener('pointerup', stopHold);
        audioBtn.addEventListener('pointerleave', stopHold);

        // --- End Start Button Listeners ---

        // Audio handler to start playback on first user interaction
        const audioStarter = () => {
            AudioEngine.init();
        ...
        document.addEventListener('keydown', (e) => {
            // Removed "aron" keyboard cheat logic
        ...
        document.addEventListener('mousemove', (e) => {
            if (e.target.closest('.btn') || e.target.closest('#audio-btn')) return; // Ignore if over a button
        ...
        document.addEventListener('touchstart', (e) => {
            ...
            if (e.target.closest('.btn') || e.target.closest('#audio-btn')) return;
        ...
        function newShip() {
            // structureHP se inicializa en 3, y la lógica de loop lo ajustará a 9 si el score es Tier 8+
            return {
                a: 90 / 180 * Math.PI,
                r: SHIP_SIZE / 2,
                ...
                mass: 20,
                invincible: false,
                structureHP: STANDARD_SHIP_HP // 3 capas por defecto
            };
        }
        ...
        function shootLaser() {
            ...
            shotCount++; // Increment successful shot counter
        ...
        /**
         * Dibuja los iconos de vidas restantes o el símbolo de infinito.
         */
        function drawLives() {
            livesDisplay.innerHTML = ''; // Limpiar iconos anteriores

            if (ship && ship.invincible) {
                // SOLICITADO POR EL USUARIO: Símbolo de infinito para vida ilimitada
                const infinity = document.createElement('span');
                infinity.classList.add('infinity-symbol');
                infinity.innerText = '∞';
                livesDisplay.appendChild(infinity);
                return;
            }
            ...
            for (let i = 0; i < maxLives; i++) {
                ...
                // Si el índice es mayor o igual a las vidas actuales, la nave se considera "perdida"
                if (i >= lives) {
        ...
        function hitShip(damageAmount, sourceIsNearPlanet = false) {
            ...
            // 1. Inmunidad Total del Modo Épico (Solo si invincible=true)
            if (ship.invincible) {
                // Si es invencible, solo aplica daño si la fuente es un planeta cercano (para forzar knockback/miedo)
                if (!sourceIsNearPlanet) return;
            }

            // 2. Reducir estructura (un golpe = una capa de defensa)
            ship.structureHP--;
        ...
        function killShip() {
            // Handles player death and respawn/game over state
            // Viewport position for explosion
            const vpX = width / 2; const vpY = height / 2;
            createExplosion(vpX, vpY, 60, '#0ff', 3);
            AudioEngine.playExplosion('large');
            ship.dead = true;

            ship.invincible = false; // Deshabilitar invencibilidad al morir

            lives--;
            drawLives(); // Update visual lives display
            ...
            if (lives > 0) setTimeout(() => {
                ...
                if (instructions) {
                    ...
                    const gameOverText = "Communication with the pilot interrupted. Vital signs interrupted. Rest in eternity.\n\n2025\n\nArón Galdón Ginés";
        ...
        function loop() {
            ...
            if (!ship.dead) {
                ...
                if (ship.blinkNum % 2 === 0) { // Invulnerability blink effect
                    ...
                    if (ship.invincible) {
                        shieldRadius = ship.effectiveR * EPIC_SHIELD_FACTOR; // Modo truco
                        shieldAlpha = 0.9;
                        strokeWidth = 3;
                    } else if (tier >= 8) {
                    ...
                    if (shieldAlpha > 0) { // Dibuja si hay HP estructural restante (capas 9-4 o HP=3), Tesla, o Invencible
                        ...
                        if (ship.invincible) {
                            baseColor = '#00ff00'; shadowColor = '#00ff00';
                        } else if (ship.structureHP <= 3 && tier < 8) {
        ...
        function startGame() {
            ...
            // Mantener la vida si el modo invencible está activo (ej. si se presiona REINICIAR)
            const wasInvincible = ship ? ship.invincible : false;
            ...
            stationSpawnTimer = 300;
            shotCount = 0; // Reset shot counter on game start
            stationsDestroyedCount = 0;
            playerReloadTime = 0; // Reset reload timer
            scoreEl.innerText = score;

            ship = newShip();
            ship.invincible = wasInvincible; // Restaurar invencibilidad si estaba activa
